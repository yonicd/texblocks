...
title / (x2 + x3)%>%
  hline()%>%
  tabular()%>%
  texPreview::texPreview(stem = 'aes-01')

title / (x2 + x3)%>%
  hline(lines = c(2,3))%>%
  tabular()%>%
  texPreview::texPreview(stem = 'aes-02')
...
purrr::reduce(rep(x1,4),`/`)%>%
  cline(l)%>%
  tabular()%>%
  texPreview::texPreview(stem = 'aes-03')

purrr::reduce(rep(x1,4),`/`)%>%
  cline(d)%>%
  tabular()%>%
  texPreview::texPreview(stem = 'aes-04')
...
purrr::reduce(rep(x1,4),`/`)%>%
  ...
  cline(d)%>%
  tabular()%>%
  texPreview::texPreview(stem = 'aes-05')
...
(title / (multirow('$\\beta$',2) + k))%>%
  tabular()%>%
  texPreview::texPreview(stem = 'aes-06')
